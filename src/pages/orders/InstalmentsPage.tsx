...
interface Instalment {
    id: string; order_id: string | null; amount: number | null; date: string; paid: boolean | null; paid_date?: string | null
    order?: { id: string; code: string; total: number | null; customer?: { first_name: string | null; last_name: string | null } | null } | null
}
...
export default function InstalmentsPage() {
    ...
    const filtered = instalments.filter(i => {
        ...
        if (filter === 'overdue') return !i.paid && (i.date || '') < today
        return true
    })
    ...
    const stats = {
        ...
        overdue: instalments.filter(i => !i.paid && (i.date || '') < today).length,
        totalPending: instalments.filter(i => !i.paid).reduce((sum, i) => sum + (i.amount || 0), 0),
        totalPaid: instalments.filter(i => i.paid).reduce((sum, i) => sum + (i.amount || 0), 0)
    }

    const getStatusIcon = (instalment: Instalment) => {
        if (instalment.paid) return <Check className="h-4 w-4 text-green-500" />
        if ((instalment.date || '') < today) return <AlertCircle className="h-4 w-4 text-red-500" />
        return <Clock className="h-4 w-4 text-yellow-500" />
    }

    const getStatusClass = (instalment: Instalment) => {
        if (instalment.paid) return 'bg-green-100 text-green-700'
        if ((instalment.date || '') < today) return 'bg-red-100 text-red-700'
                ...
                {loading ? <div className="text-center py-8">Loading...</div> : filtered.length === 0 ? (
                    ...
                    <table className="w-full"><thead><tr className="border-b"><th className="text-left p-3">Order</th><th className="text-left p-3">Customer</th><th className="text-right p-3">Amount</th><th className="text-left p-3">Due Date</th><th className="text-center p-3">Status</th><th className="text-center p-3">Actions</th></tr></thead>
                        <tbody>{filtered.map(i => (
                            <tr key={i.id} className={`border-b hover:bg-[hsl(var(--muted))] ${!i.paid && (i.date || '') < today ? 'bg-red-50' : ''}`}>
                                ...
                                <td className="p-3 text-right font-bold text-[hsl(var(--primary))]">{formatCurrency(i.amount || 0)}</td>
                                <td className="p-3"><div className={(i.date || '') < today && !i.paid ? 'text-red-600 font-bold' : ''}>{new Date(i.date || '').toLocaleDateString()}</div>{i.paid_date && <div className="text-xs text-green-600">Paid: {new Date(i.paid_date).toLocaleDateString()}</div>}</td>
                                <td className="p-3 text-center"><span className={`inline-flex items-center gap-1 px-2 py-1 rounded text-xs ${getStatusClass(i)}`}>{getStatusIcon(i)}{i.paid ? 'Paid' : (i.date || '') < today ? 'Overdue' : 'Pending'}</span></td>
            ...
            {stats.pending > 0 && (
                <Card><CardHeader><CardTitle>Upcoming Due Dates</CardTitle></CardHeader><CardContent>
                    <div className="space-y-2">
                        {instalments.filter(i => !i.paid).sort((a, b) => (a.date || '').localeCompare(b.date || '')).slice(0, 10).map(i => {
                            const daysUntil = Math.ceil((new Date(i.date || '').getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24))
                            return <div key={i.id} className={`flex items-center justify-between p-3 rounded ${daysUntil < 0 ? 'bg-red-100' : daysUntil <= 3 ? 'bg-yellow-100' : 'bg-[hsl(var(--muted))]'}`}>
                                ...
                                <div className="flex items-center gap-4">
                                    <span className="font-bold">{formatCurrency(i.amount || 0)}</span>
